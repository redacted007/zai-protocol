...
const INITIAL_STAKE_MULTIPLE = new BN(10).pow(new BN(6)) // 100 ZAI -> 100M ZAIS

const FROZEN = new BN(0)
const FLUID = new BN(1)

async function incrementEpoch(dao, num = 1) {
  await dao.set((await dao.epoch()).toNumber() + num)
}
...
describe('Pool', function () {
  ...
  beforeEach(async function () {
    ...
    this.dollar = await MockToken.new('Zero Collateral Dai', 'ZAI', 18, {
      from: ownerAddress,
      gas: 8000000,
    })
  ...
  describe('frozen', function () {
    ...
    describe('when claim', function () {
      beforeEach(async function () {
        ...
        await this.dao.set((await this.dao.epoch()) + 16)
      ...
      describe('simple', function () {
        ...
        it('updates dao balances', async function () {
          ...
          expect(await this.pool.totalClaimable()).to.be.bignumber.equal(
            new BN(0),
          )
          expect(await this.pool.totalRewarded()).to.be.bignumber.equal(
            new BN(0),
          )
        })
      ...
      describe('with reward', function () {
        ...
        describe('multiple without reward first', function () {
          ...
          it('updates users balances', async function () {
            ...
            expect(
              await this.pool.balanceOfRewarded(userAddress1),
            ).to.be.bignumber.equal(new BN(600).mul(INITIAL_STAKE_MULTIPLE))
            expect(
              await this.pool.balanceOfPhantom(userAddress1),
            ).to.be.bignumber.equal(new BN(600).mul(INITIAL_STAKE_MULTIPLE))
            expect(
              await this.pool.balanceOfRewarded(userAddress2),
            ).to.be.bignumber.equal(new BN(400).mul(INITIAL_STAKE_MULTIPLE))
            expect(
              await this.pool.balanceOfPhantom(userAddress2),
            ).to.be.bignumber.equal(new BN(400).mul(INITIAL_STAKE_MULTIPLE))
            expect(
            ...
            expect(
              await this.pool.balanceOfPhantom(userAddress),
            ).to.be.bignumber.equal(new BN(1000).mul(INITIAL_STAKE_MULTIPLE))
          })
      ...
      describe('with reward', function () {
        ...
        describe('partial', function () {
          ...
          it('updates users balances', async function () {
            ...
            expect(
              await this.pool.balanceOfPhantom(userAddress),
            ).to.be.bignumber.equal(new BN(200).mul(INITIAL_STAKE_MULTIPLE))
          })
    ...
    describe('when provide', function () {
      beforeEach(async function () {
        ...
        this.poolLockupEpochs = 144
        await incrementEpoch(this.dao, this.poolLockupEpochs)
        await this.dollar.mint(this.pool.address, 1000)
      })
  ...
  describe('when emergency withdraw', function () {
    ...
    describe('as dao', function () {
      ...
      it('transfers funds to the dao', async function () {
        expect(await this.univ2.balanceOf(mockDao)).to.be.bignumber.equal(
          new BN(1000),
        )
        expect(
          await this.univ2.balanceOf(this.pool.address),
        ).to.be.bignumber.equal(new BN(0))
        expect(await this.dollar.balanceOf(mockDao)).to.be.bignumber.equal(
          new BN(1000),
        )
        expect(
          await this.dollar.balanceOf(this.pool.address),
