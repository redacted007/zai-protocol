...
describe('Bonding', function () {
    ...
    describe('when bond', function () {
      ...
      describe('partial', function () {
        ...
        it('updates users balances', async function () {
          ...
          expect(
            await this.bonding.balanceOf(userAddress),
          ).to.be.bignumber.equal(new BN(500).mul(INITIAL_STAKE_MULTIPLE))
          expect(
            await this.bonding.balanceOfStaged(userAddress),
      ...
      describe('multiple', function () {
        ...
        it('updates users balances', async function () {
          ...
          expect(
            await this.bonding.balanceOf(userAddress),
          ).to.be.bignumber.equal(new BN(250).mul(INITIAL_STAKE_MULTIPLE))
          expect(
            await this.bonding.balanceOfStaged(userAddress),
          ).to.be.bignumber.equal(new BN(300))
          expect(
  ...
  describe('when step', function () {
    beforeEach(async function () {
      ...
      await time.increase(86400)
      const DAO_LOCKUP = 239
      for (var i = 0; i < DAO_LOCKUP; i++) {
        await this.bonding.stepE({ from: userAddress })
      }
    })
    ...
    describe('preceeding epoch cooldown', function () {
      ...
      it('is correct epoch', async function () {
        expect(await this.bonding.epoch()).to.be.bignumber.equal(new BN(242))
      })
    })
    ...
    describe('after epoch lock cooldown', function () {
      ...
      it('is correct epoch', async function () {
        expect(await this.bonding.epoch()).to.be.bignumber.equal(new BN(243))
      })
